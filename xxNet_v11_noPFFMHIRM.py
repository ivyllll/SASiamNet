...
class Decoder(nn.Module):
    ...
    def forward(self, d5, d4, d3, d2):
        d5 = F.interpolate(d5, d4.size()[2:], mode='bilinear',
                           align_corners=True)  # 要把d5的HW变成和d4一样大小才能conv_sum(也就是fusion)
        d5 = self.eca(d5)
        # d5 = self.ca(d5)
        # d5 = d5 * self.ca(d5)
        ...
        d4 = F.interpolate(d4, d3.size()[2:], mode='bilinear',
                           align_corners=True)  # 要把d4的HW变成和d3一样大小才能conv_sum(也就是fusion)
        d4 = self.eca(d4)
        # d4 = self.ca(d4)
        # d4 = d4 * self.ca(d4)
        ...
        d3 = F.interpolate(d3, d2.size()[2:], mode='bilinear',
                           align_corners=True)  # 要把d3的HW变成和d2一样大小才能conv_sum(也就是fusion)
        d3 = self.eca(d3)
        # d3 = self.ca(d3)
        # d3 = d3 * self.ca(d3)
...
class BaseNet(nn.Module):
    def __init__(self, input_nc, output_nc):
        ...
        self.mid_d = 64
        # self.RFEM5 = TemporalFeatureInteractionModule(512, self.mid_d)
        # self.RFEM4 = TemporalFeatureInteractionModule(256, self.mid_d)
        # self.RFEM3 = TemporalFeatureInteractionModule(128, self.mid_d)
        # self.RFEM2 = TemporalFeatureInteractionModule(64, self.mid_d)
    ...
    def forward(self, x1, x2):
        ...
        # x = torch.cat((x1, x2), 0)  # [3, 512, 512]
        x = torch.add(x1, x2)
        d1, d2, d3, d4, d5 = self.backbone.base_forward(x)
        # [64,256,256] [64,128,128] [128,64,64] [256,32,32] [512,16,16]
        d5 = self.conv5(d5)
        d4 = self.conv4(d4)
        d3 = self.conv3(d3)
        d2 = self.conv2(d2)
        ...
        # d5_p, d4_p, d3_p, d2_p = self.CIEM1(d5, d4, d3, d2, d5, d4, d3, d2)  # 去掉PFFM(终版)
