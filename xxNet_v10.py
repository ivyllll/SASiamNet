...
class CIEM(nn.Module):
    ...
    def forward(self, r5, r4, r3, r2):
        # upsampling
        r5_ = F.interpolate(r5, r2.size()[2:], mode='bilinear', align_corners=True)  # 插值法进行上采样,改变特征图size,使它们大小相同
        r4_ = F.interpolate(r4, r2.size()[2:], mode='bilinear', align_corners=True)
        r3_ = F.interpolate(r3, r2.size()[2:], mode='bilinear', align_corners=True)
        r2_ = r2
        ...
        d2 = self.conv_concatenate(torch.cat([d2, r2], dim=1))
        d3 = self.conv_concatenate(torch.cat([d3, r3], dim=1))
        d4 = self.conv_concatenate(torch.cat([d4, r4], dim=1))
        d5 = self.conv_concatenate(torch.cat([d5, r5], dim=1))
        return d5, d4, d3, d2
...
class FeatureFusionModule(torch.nn.Module):
    def __init__(self, num_classes):
        ...
        self.convblock = nn.Sequential(
            nn.Conv2d(in_channels=self.in_channels, out_channels=num_classes, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(self.out_d),
            nn.ReLU(inplace=True)
        )
...
class BaseNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        r5, r4, r3, r2 = self.PFFM(dm5, dm4, dm3, dm2,
                                  d5, d4, d3, d2,
                                  am5, am4, am3, am2)
        r5, r4, r3, r2 = self.MFAM(dm5, dm4, dm3, dm2,
                                   r5, r4, r3, r2,
                                   am5, am4, am3, am2)

        # change information guided refinement 1
        # d5_p, d4_p, d3_p, d2_p = self.CIEM1(d5, d4, d3, d2)
        d5_p, d4_p, d3_p, d2_p = self.CIEM1(r5, r4, r3, r2) #加了PFFM
